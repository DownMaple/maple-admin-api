...
pub fn decrypt_password(encrypted_base64: &str) -> Result<String, AppError> {
    ...
    tracing::debug!(
        "加密密码前 50 字符: {}",
        &encrypted_base64[..encrypted_base64.len().min(50)]
    );

    // 解析私钥 (PKCS#8 格式)
    let private_key =
        RsaPrivateKey::from_pkcs8_pem(key_manager.get_private_key()).map_err(|e| {
    ...
    let decrypted_data = private_key
        ...
        .map_err(|e| {
            ...
            tracing::error!(
                "   可能原因: 1) 前后端使用了不同的密钥对  2) 密码格式错误  3) 加密算法不匹配"
            );
    ...
    // 转换为字符串
    let plain_password = String::from_utf8(decrypted_data).map_err(|e| {
